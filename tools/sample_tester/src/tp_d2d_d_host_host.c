...
int32_t tp_d2d_d_host_host(void)
{
	logfile(LOG_DEBUG, "--- test tp_d2d_d_host_host start!! ---\n");
	...
	//----------------------------------------------
	// D2D fpga direct connect
	//----------------------------------------------
	if (tp_fpga_direct_connect() < 0) {
		// error
		goto _END31;
	}
...
_END32:
	// fpga direct disconnect
	tp_fpga_direct_disconnect();
...
_END1:
	...
	logfile(LOG_DEBUG, "...test tp_d2d_d_host_host end\n");
