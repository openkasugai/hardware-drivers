...
int32_t tp_host_host(void)
{
	logfile(LOG_DEBUG, "--- test tp_host_host start!! ---\n");
	...
	//----------------------------------------------
	if (tp_shmem_allocate(SHMEM_SRC_DST, pque) < 0) {
		// error
		goto _END1;
	}
	...
	// fpga fdma init
	//----------------------------------------------
	if (tp_enqueue_fdma_init(*dev_id) < 0) {
		// error
		goto _END3;
	}
	if (tp_dequeue_fdma_init(*dev_id) < 0) {
		// error
		goto _END4;
	}
	...
	//----------------------------------------------
	if (tp_enqueue_fdma_queue_setup(*dev_id) < 0) {
		// error
		goto _END5;
	}

	if (tp_dequeue_fdma_queue_setup(*dev_id) < 0) {
		// error
		goto _END6;
	}
	...
	for (size_t r=0; r < run_num; r++) {
		...
		for (size_t i=0; i < CH_NUM_MAX; i++) {
			uint32_t ch_id = i;
			if (getopt_ch_en(ch_id)) {
				th_dma_tx_deq_args[ch_id] = (thread_deq_args_t){*dev_id, ch_id, run_id, enq_num};
		...
		for (size_t i=0; i < CH_NUM_MAX; i++) {
			uint32_t ch_id = i;
			if (getopt_ch_en(ch_id)) {
				th_dma_tx_enq_args[ch_id] = (thread_enq_args_t){*dev_id, ch_id, run_id, enq_num};
		...
		for (size_t i=0; i < CH_NUM_MAX; i++) {
			uint32_t ch_id = i;
			if (getopt_ch_en(ch_id)) {
				th_dma_rx_deq_args[ch_id] = (thread_deq_args_t){*dev_id, ch_id, run_id, enq_num};
		...
		for (size_t i=0; i < CH_NUM_MAX; i++) {
			uint32_t ch_id = i;
			if (getopt_ch_en(ch_id)) {
				th_dma_rx_enq_args[ch_id] = (thread_enq_args_t){*dev_id, ch_id, run_id, enq_num};
...
_END8:
	// chain disconnect
	tp_chain_disconnect(*dev_id);

_END7:
	// fdma queue finish
	tp_dequeue_fdma_queue_finish(*dev_id);

_END6:
	// fdma queue finish
	tp_enqueue_fdma_queue_finish(*dev_id);

_END5:
	// fdma finish
	tp_dequeue_fdma_finish(*dev_id);

_END4:
	// fdma finish
	tp_enqueue_fdma_finish(*dev_id);
...
_END1:
	...
	logfile(LOG_DEBUG, "...test tp_host_host end\n");
